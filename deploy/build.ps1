...
'@
    ...
    # Check for a prebuild script and run if present.
    Get-ChildItem -Path $PSScriptRoot -Filter *.cake |
            ...
            Foreach-Object {
                ...
                Invoke-Expression "& $DotNetExpression `"$CAKE`" `"$($_.FullName)`" -target=`"$Target`" -configuration=`"$Configuration`" -verbosity=`"$Verbosity`" $UseMono $UseDryRun $UseExperimental"

                if ($LASTEXITCODE -ne 0)
                {
                    throw "Prebuild script failed."
                }
            }
    ...
    if (-not ($Script.Contains('/') -or $Script.Contains('\') -or (Test-Path -Path $Script -PathType Leaf)))
    {
        # Assume build script is in same folder as this file
        $Script = Join-Path $PSScriptRoot $Script
    }

    Invoke-Expression "& $DotNetExpression `"$CAKE`" `"$Script`" -target=`"$Target`" -configuration=`"$Configuration`" -verbosity=`"$Verbosity`" $UseMono $UseDryRun $UseExperimental $ScriptArgs"
    exit $LASTEXITCODE
}
catch
{
